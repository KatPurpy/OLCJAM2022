...
{
	ImU32
	b2CtoImU32(const b2Color& color)
	{
	...
	ImU32
	b2pCtoIMU32(b2ParticleColor color)
	{
  ...
  public:
	...
	void
	DrawCircle(const b2Vec2& center, float radius, const b2Color& color)
	{
		auto drawlist = ImGui::GetBackgroundDrawList();
	...
	void
	DrawTransform(const b2Transform& xf)
	{
	}

	void
	DrawPoint(const b2Vec2& p, float size, const b2Color& color)
	{
		auto drawlist = ImGui::GetBackgroundDrawList();
	...
	void
	DrawParticles(const b2Vec2* centers, float32 radius, const b2ParticleColor* colors, int32 count)
	{
		auto drawList = ImGui::GetBackgroundDrawList();
