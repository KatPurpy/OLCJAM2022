...
#include "stb_ds.h"
#include "stdio.h"
template <class T, Constants::PhysicsCategory type, bool allowNullData> class OverlapCheck
{
  ...
  public:
	...
	void
	GenericOverlap(b2World* world, b2BodyDef* def, shapeT* shape)
	{
		arrsetlen(cache, 0);
		b2FixtureDef fdef;
		fdef.isSensor = true;
	...
	T*
	OverlapCircle(b2World* world, b2Vec2 vec, float radius)
	{
		b2BodyDef def;
		def.awake = true;
